...
终端展示统一由 CLI 创建的 `AgentDisplay` 接收这两个回调；普通状态、verbose 诊断、计时与换行顺序都在 `display.py` 中处理。`run_agent` 不接收 `verbose` 参数，也不创建终端输出器；程序调用者需要详细终端展示时，创建 `AgentDisplay(verbose=True, secrets=...)`，传入它的 `content` 和 `event` 方法。CLI 的 `--verbose` 用法不变。实验采集器可独立接收 `on_event`，不必启用终端展示。诊断流写入/刷新失败可以忽略，正文写入失败仍向上传播；这些约定不改变模型消息和研究决策。
...
未增加独立 chat/stream 产品开关，不实现关键词去噪、语义去重或额外来源数据库。事实依据和保留问题见 [研究结论](research-findings.md)，运行证据见 [验收记录](validation.md)。
...
- **临时会话与成功提交**：一次输入可能多轮工具执行；中间失败不应污染后续对话。因此消息和页面快照一起复制、一起提交，代价是长会话复制和上下文成本，不支持并发共享。
- **完整工具响应后执行**：工具参数按分片累计，收到完整 finish_reason 和 DONE 后才校验执行，避免截断参数触发操作。正文可先显示，但它的可见性不等于会话已提交。
- **搜索与读取分开**：搜索提供候选与片段，read_page 核查明确 URL；失败不暗中搜索替代页面，保持来源归因可解释。
- **显示使用回调**：Agent 只提供内容和生命周期事件，CLI 连接 AgentDisplay。实验采集与终端显示可各自消费，verbose 格式不会改变模型决策输入。
- **保守处理证据**：只做字段校验、空白整理和单次精确去重。噪声与重复是否有害取决于任务；小样本尚不支持全局语义清理。
- **显式笔记保持实验性**：结构化字段与引用校验有助于观察，但未证明质量收益，故默认自由笔记，不另建笔记数据库。
